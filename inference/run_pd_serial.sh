...
CKPT_PATH="/mnt/data1/modelscope/deepseek-ai/DeepSeek-V4-Flash-w8a8"
CONFIG="$ROOT/config_w8a8.json"
...
DEEPSEEK_CPU_MOE_SHARED_WEIGHT_DIR="${DEEPSEEK_CPU_MOE_SHARED_WEIGHT_DIR:-/tmp/dsv4_shared_cpu_moe_${$}}"
...
start_external_server() {
  ...
  local server_log="/tmp/dsv4_cpu_moe_server_${$}.log"
  env \
...
text = "\n".join(parts) + "\n请用三句话总结以上内容。"
with open("/tmp/dsv4_long_input_single.txt", "w") as f:
    f.write(text)
print(f"wrote /tmp/dsv4_long_input_single.txt tokens={len(tok.encode(text))}", flush=True)
PY
fi
...
run_single() {
  ...
  PYTHONPATH="$ROOT" \
  "${client_launcher[@]}" --master-port "$master_port" --nproc-per-node 4 "$ROOT/generate.py" \
    --ckpt-path "$CKPT_PATH" \
    --config "$CONFIG" \
...
run_suite() {
  ...
  if [[ "$PD_CASE" == "both" || "$PD_CASE" == "short" ]]; then
    ...
    run_single "$mode" "$SHORT_INPUT_FILE" "$SHORT_MAX_NEW_TOKENS" "$short_master_port" "/tmp/dsv4_pd_short_client_${suffix}.log"
  fi

  if [[ "$PD_CASE" == "both" || "$PD_CASE" == "long" ]]; then
    echo "=== long sequence correctness/performance (pd-mode=$mode) ==="
    run_single "$mode" "$LONG_INPUT_FILE" "$LONG_MAX_NEW_TOKENS" "$long_master_port" "/tmp/dsv4_pd_long_client_${suffix}.log"
  fi
}
