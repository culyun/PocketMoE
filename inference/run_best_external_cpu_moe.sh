...
if [[ "${DEEPSEEK_SKIP_BENCHMARK_LOCK:-0}" != "1" ]]; then
  LOCK_FILE="${DEEPSEEK_FULL_NETWORK_BENCHMARK_LOCK:-/tmp/dsv4_full_network_benchmark.lock}"
  exec 9>"$LOCK_FILE"
  if ! flock -n 9; then
...
INPUT_FILE="${INPUT_FILE:-$ROOT/smoke_input.txt}"
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-8}"
MASTER_PORT="${MASTER_PORT:-29661}"
SHM_NAME="${DEEPSEEK_CPU_MOE_SERVER_SHM:-dsv4_cpu_moe_server}"
SERVER_LOG="${SERVER_LOG:-/tmp/dsv4_cpu_moe_server_best.log}"
CLIENT_LOG="${CLIENT_LOG:-/tmp/dsv4_best_external_cpu_moe.log}"
...
cleanup() {
  PYTHONPATH="$ROOT" "$PYTHON" - <<PY || true
...
PYTHONPATH="$ROOT" \
"$TORCHRUN" --master-port "$MASTER_PORT" --nproc-per-node 4 "$ROOT/generate.py" \
  --ckpt-path "$CKPT_PATH" \
  --config "$CONFIG" \
