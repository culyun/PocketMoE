...
sample_rss() {
  ...
  while [[ ! -e "$stop_marker" ]]; do
    local ts pids
    ts=$(date +%s)
    # Match every running generate.py worker (4 ranks). torchrun (the parent)
    # also carries "generate.py" in its cmdline. We sum all matches since the
    # workers hold the actual model state.
    pids=$(pgrep -f "generate.py" || true)
    if [[ -n "$pids" ]]; then
      local sum=0
      local rank_count=0
      for p in $pids; do
