...
_EXT_PATH = Path(__file__).with_name("cuda_kernel.so")
_NATIVE_MOD = None
...
def _find_built_extension() -> Optional[Path]:
    ...
    for suffix in suffixes:
        candidate = Path(__file__).with_name(f"cuda_kernel{suffix}")
        if candidate.exists():
            return candidate
    return None
...
def load_cuda_kernel() -> Optional[object]:
    ...
    try:
        _preload_torch_libs()
        spec = importlib.util.spec_from_file_location("cuda_kernel", ext_path)
